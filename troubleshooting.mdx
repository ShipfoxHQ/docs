...
<AccordionGroup>
  <Accordion title="Run is stuck in `pending`">
    ...
    **Fix:**
    - Check **Settings → Runners** to see which runners are online and their labels.
    ...
    - In evaluation mode, confirm `docker compose up -d` is running.
  </Accordion>
  ...
  <Accordion title="Workflow file not detected">
    ...
    **Fix:**
    - Confirm the file is under `.shipfox/workflows/` (not in a subdirectory).
  ...
  <Accordion title="Validation error on push">
    ...
    **Common mistakes:**
    - Using `loop`, `matrix`, `branch`, or `${{ }}` — these are roadmap features not yet shipped.
  ...
  <Accordion title="Agent step fails or produces no output">
    ...
    **Fix:**
    - Verify the AI provider is configured in **Settings → AI Providers** with a valid API key.
  ...
  <Accordion title="Push trigger not firing">
    **Cause:** GitHub integration not connected, or repository is on GitLab.
  ...
  <Accordion title="`restart_from` validation error">
    **Cause:** The step named in `restart_from` doesn't exist, appears after the gate step, or has no `name:`.

    **Fix:**
    - Add a `name:` field to the step you want to restart from.
    - Ensure the named step appears **before** the gate step in the same job.
