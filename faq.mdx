...
<AccordionGroup>
  ...
  <Accordion title="Can I use `${{ }}` template expressions in my workflow?">
    Not yet — `${{ }}` interpolation is roadmap. All `run` commands and `prompt` strings are treated as literals today. Write static values.
  </Accordion>
  ...
  <Accordion title="How do I run Shipfox locally to test?">
    Use the Docker Compose evaluation stack. See the [Installation guide](/installation) for step-by-step instructions. Note that push triggers don't fire locally — use manual triggers for local testing.
  </Accordion>
  ...
  <Accordion title="Is self-hosting supported?">
    Yes. Shipfox requires PostgreSQL, a workflow orchestration service, and S3-compatible object storage. The runner is a separate process you deploy on your compute. See the [Installation guide](/installation) for details. Managed hosting is also available.
  </Accordion>

  <Accordion title="What happens when a gate's `restart_from` loops infinitely?">
    Set `execution_timeout` on the job (e.g., `"30m"`) to cap total job duration. Without a timeout, a loop can run until the job hits the system maximum.
  </Accordion>
