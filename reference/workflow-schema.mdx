...
<ParamField body="source" type="string" required>
  The integration source that produces this trigger (e.g., `github`, `sentry`, `manual`). See the [Trigger Sources reference](/reference/trigger-sources) for all supported values.
</ParamField>
...
<ParamField body="filter" type="string">
  A CEL expression that narrows which events fire this trigger (e.g., `event.ref == "main"`). The expression is parsed and stored but **not yet evaluated** — the trigger fires on every matching `(source, event)` pair regardless of filter content.
</ParamField>
...
<ParamField body="name" type="string">
  Displayed in the dashboard log. Required if this step is referenced by `restart_from` in a gate block.
</ParamField>
...
<ParamField body="on_failure.restart_from" type="string">
  The `name` of an earlier step in the same job to restart from when this step fails. The referenced step must be a named step that appears before this step in the job's step list.
</ParamField>
...
<Warning>
  ...
  - **Unknown keys are rejected everywhere.** All schema objects use `strictObject`; any unrecognised key is an error.
  ...
  - **`restart_from` must name an earlier named step in the same job.** The referenced step must appear before the gate-bearing step in the step list and must have a `name`.
