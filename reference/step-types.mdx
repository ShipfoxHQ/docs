...
<ParamField body="name" type="string">
  Displayed in the dashboard log. Required if this step is referenced in a `restart_from` field elsewhere in the same job.
</ParamField>
...
<ParamField body="on_failure.restart_from" type="string">
  The `name` of an earlier step in the same job to restart from when this step fails. The referenced step must be named and must appear before the current step in the job's step list.
</ParamField>
