---
title: "Gate Conditions and Retry Loops in Shipfox"
...
A **gate** block on a step lets you define a success condition and a retry strategy. When the condition fails, Shipfox restarts execution from an earlier named step in the same job — creating a loop that continues until the condition is met or the job times out. This pattern is especially powerful when paired with agent steps: the agent edits code, a shell step verifies it, and the gate loops back if the verification fails.
...
gate:
  ...
  on_failure:
    restart_from: install      # must name an earlier step that has a `name:` field
```
...
On each iteration, the agent reads the updated codebase and attempts another fix. The loop continues until `npm test` exits `0`. If the job's `execution_timeout` is reached before the condition is met, the job fails.
...
<Warning>
  The step named in `restart_from` must appear **before** the gate step in the same job. Forward references are not allowed.
</Warning>

- The target step must have a `name:` field. Unnamed steps cannot be referenced by `restart_from`.
- The reference is matched by step name, not by position — reordering steps changes which step is targeted.
...
Gates create loops. Without a timeout, a loop where the agent never converges on a fix will run until the workflow is cancelled manually. Set `execution_timeout` on the job to establish an upper bound:
