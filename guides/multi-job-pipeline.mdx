---
title: "Build Multi-Job Pipelines in Shipfox"
sidebarTitle: "Multi-Job Pipeline"
description: "Connect Shipfox jobs with needs to create sequential or parallel pipelines. Each job runs in isolation and re-clones the repo — learn how to structure them."
---
...
<Warning>
  Jobs do not share a filesystem. Even though `install` runs `npm ci`, the
  `test` job must also run `npm ci` independently — each job re-clones the
  repository from scratch into a fresh environment. There is no shared
  `node_modules` between jobs.
</Warning>
