...
description: "Shipfox's core artifact is workflow YAML that coding agents write. These docs are AI-ingestible — every page is available as Markdown for agents to browse and cite."
---
...
<CardGroup cols={3}>
  <Card title="AI-Ingestible Docs" icon="file-lines" href="/ai/overview">
...
The most common mistake when writing Shipfox YAML is using roadmap-only keys (`loop`, `matrix`, `${{ }}`), mixing `run` and `prompt` on one step, or setting `env` on an agent step. These errors are easy to make when working from memory or incomplete documentation — and they all result in rejected workflows at validation time.
...
<Note>
  A Shipfox plugin for Claude Code, Cursor, and Codex is planned — with slash commands (`/shipfox:new-workflow`, `/shipfox:validate`, `/shipfox:run`), specialist sub-agents, and session activation for repos with `.shipfox/`. Ship the skills package today; the plugin lands when the YAML surface stabilizes.
</Note>
