---
title: "Shipfox Authoring Prompt for Coding Agents"
...
Trigger { source, event, filter?, with? }. Shipped: manual/fire, github/push,
sentry/issue.(created|resolved|assigned|archived|unresolved). At most one manual
trigger.

Job { steps (>=1), needs?, runner?, env? }. Jobs run in parallel; each is isolated
and re-clones the repo (install deps per job).

Step is a run step { run, name?, env?, gate? } OR an agent step
{ model, prompt, thinking?, provider?, name?, gate? } — never both; agent requires
prompt; env is run-only.

gate { success_if: "<CEL over exit_code>", on_failure: { restart_from: "<earlier
named step>" } }.

Do NOT use (not shipped): loop, matrix, branch, structured output, reusable agents,
and ${{ }} interpolation (prompts/run are literal). filter isn't enforced yet.
event.ref is the bare branch name (e.g. "main").
```
...
- **Trigger sources (shipped today)** — `manual/fire`, `github/push`, and `sentry/issue.*` events
...
- **Hard rules** — no `loop`, no `${{ }}` interpolation, `env` is run-only, at most one manual trigger
