...
gate (run steps only) { success_if: "<CEL over exit_code>", on_failure: {
restart_from: "<earlier named step>", output?: "<message>" } }. Restarts are capped
(3 attempts per gating step) before the job fails.
