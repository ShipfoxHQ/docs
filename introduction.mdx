---
title: "What is Shipfox? Agentic Workflows Platform"
...
<Steps>
  ...
  <Step title="Configure a trigger">
    Choose when the workflow runs: on a GitHub push, a Sentry issue, a schedule, or on demand directly from the dashboard. Each trigger is a named entry under the `triggers` key in your workflow YAML.
  </Step>
  ...
  <Step title="Steps: commands or agents">
    Each job contains one or more steps. A step either runs a shell command (`run`) or invokes an AI model by specifying a `model` and a `prompt`. Both step kinds share the same lifecycle: they stream output, report status, and can be gated on success conditions.
  </Step>
...
<CardGroup cols={2}>
  <Card title="Checks on every push" icon="circle-check" href="/guides/checks-on-push">
    Run tests, lint, and get an AI review on every commit — automatically, without configuring a separate CI system.
  </Card>
...
| GitHub push triggers (`source: github, event: push`) | ✅ Shipped |
...
| Expression interpolation (`${{ }}`) | 🔜 Roadmap |
