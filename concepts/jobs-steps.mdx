---
title: "Jobs and Steps: How Shipfox Executes Work"
sidebarTitle: "Jobs & Steps"
description: "Jobs group steps into isolated units of work. Steps are either shell commands or AI agent invocations. Learn how Shipfox executes both inside a workflow run."
---
...
```yaml
steps:
  - name: install          # optional — names the step for restart_from
    run: npm ci
    env:
      CI: "true"
    gate:
...
<ParamField path="name" type="string">
  A human-readable name for the step. Also used as the target identifier for `gate.on_failure.restart_from` — if you want a gate to be able to restart from this step, it must have a `name`.
</ParamField>
...
<ParamField path="prompt" type="string" required>
  The instruction sent to the AI model. Write literal text — template expressions are not evaluated yet.
</ParamField>
...
<ParamField path="name" type="string">
  Human-readable name for the step. Used by gate `restart_from` references.
</ParamField>
...
A gate block lets a step evaluate its own success and optionally restart the job from an earlier named step on failure.

```yaml
gate:
  success_if: exit_code == 0          # CEL expression
  on_failure:
    restart_from: install             # must name an earlier step in the same job
```
...
<ParamField path="on_failure.restart_from" type="string">
  The `name` of an earlier step in the same job to restart from when this step's gate is not satisfied. The referenced step must appear before the current step in the `steps` list and must have a `name` field.
</ParamField>
