---
title: "Workflows: YAML Automation Files in Your Repo"
...
- **Subdirectories are not scanned.** All workflow files must be placed directly under `.shipfox/workflows/`. A file at `.shipfox/workflows/ci/build.yml` will not be picked up.
...
- **Values are literal.** The `${{ }}` template expression syntax is not evaluated — if you write `${{ github.sha }}`, that exact string is passed to the shell. Use literal values today.
...
<Note>
  Template expressions (`${{ ... }}`), `filter` evaluation, `loop`, `matrix`, and `branch` are roadmap features. Today, write literal values in prompts and `run` scripts.
</Note>
