...
A **gate** turns a run step's exit code into a decision: did this step succeed, and
...
any external scripting or orchestration.
...
<ParamField path="gate.on_failure.restart_from" type="string">
  The `name` of an earlier step in the same job to restart from when the gate
  fails. Execution resumes at that step.
</ParamField>
...
<Note>
  Gates apply to **run steps only**. The schema rejects a `gate` on an agent step.
  To act on an agent's result, follow it with a `run` step that inspects the output
  and gate on that.
</Note>
